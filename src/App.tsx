...
export const App = () => {
  ...
  const initialMessages = { text: 'Hi, what would you like to search on?' }
  ...
  const connect = {
    ...
    try {
        ...
        websocket.onmessage = (message) => {
          ...
          // If Stop Key is received, stop the stream
          if (response.text === 'stop-key')
          {
      ...
      websocket.onerror = () => {
       ...
       signals.newUserMessage.listener = (body) => {
          ...
          if(lastQueryResults.current != null && lastMessage.text.toLowerCase() === 'generate synthesis') {   
            body.records = lastQueryResults.current;
            body.generate_synthesis = true;
          }
